...
`package.json` の `version` が単一の情報源（vite.config.ts の define で `__APP_VERSION__` として
埋め込み、ヘッダー右上にピル表示）。**バージョンはビューア本体の変化のみ**を表し、データの鮮度は
既存の「取得日」表示が担う。

- **PATCH**: バグ修正・文言や見た目の調整・性能改善
- **MINOR**: 機能追加・表示情報の追加・**収録法令の追加**（引ける範囲の拡大=機能扱い）
- **MAJOR**: **ユーザーが判断する（Claudeは勝手に上げない）**。URLハッシュ形式やデータモデルの
  非互換変更、または全法令リアルタイム検索・APIオンデマンド取得・複数法令の横断検索などの
  大型アップデートが候補
- 週次の自動データ更新（Actions）ではバージョンを上げない
- 修正依頼1件の完了ごとに、対応する桁を**同じコミットで**上げる。
  複数の修正が混在する場合は最も大きい桁を1つだけ上げる（MINOR相当が1つでもあれば MINOR）
...
全法令検索・オンデマンド取得・横断検索の実現時はメジャーバージョンアップ候補（判断はユーザー）。
